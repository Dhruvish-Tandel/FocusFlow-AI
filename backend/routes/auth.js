...
router.post('/register', async (req, res) => {
  try {
    const { name, email, password, studyHoursPerDay, preferredStartTime } = req.body;
    const exists = await User.findOne({ email });
    if (exists) return res.status(400).json({ message: 'User already exists' });

    const user = await User.create({ name, email, password, studyHoursPerDay, preferredStartTime });
